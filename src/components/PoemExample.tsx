...
export function PoemExample({ line, poemTitle, author, fullPoem }: PoemExampleProps) {
  ...
  const hasPoem = Boolean(poemTitle && fullPoem && poemTitle !== "Unknown" && poemTitle.trim() !== "");
  ...
  return (
    <div className="bg-white/70 rounded-lg p-4 border border-[#00209F]/20">
      <div className="space-y-3">
        ...
        {hasPoem && (
          <div className="flex flex-col md:flex-row md:items-center gap-y-4 justify-between pt-2 border-t border-[#00209F]/10">
            <div className="text-sm text-slate-600">
              <p className="font-medium">— {poemTitle}</p>
              {author && <p>pa {author}</p>}
            </div>
                ...
                <DialogHeader>
                  <div className="flex items-start justify-between gap-4 pr-8">
                    <div className="flex-1">
                      ...
                      {author && <p className="text-lg text-left text-slate-600 mt-1">pa {author}</p>}
                    </div>
